...
func TestCLI_BumpPreCmd(t *testing.T) {
	...
	tests := []struct {
		...
		{"increment rc.1 to rc.2", "1.2.3-rc.1", []string{"semver", "bump", "pre"}, "1.2.3-rc.2"},
		{"increment rc.9 to rc.10", "1.2.3-rc.9", []string{"semver", "bump", "pre"}, "1.2.3-rc.10"},
		{"increment alpha.1 to alpha.2", "2.0.0-alpha.1", []string{"semver", "bump", "pre"}, "2.0.0-alpha.2"},

		// Increment existing pre-release (no separator)
		{"increment rc1 to rc2", "1.2.3-rc1", []string{"semver", "bump", "pre"}, "1.2.3-rc2"},
		{"increment beta5 to beta6", "1.2.3-beta5", []string{"semver", "bump", "pre"}, "1.2.3-beta6"},

		// Increment existing pre-release (dash separator)
		{"increment rc-1 to rc-2", "1.2.3-rc-1", []string{"semver", "bump", "pre"}, "1.2.3-rc-2"},

		// Label switch
		{"switch from alpha to beta", "1.2.3-alpha.3", []string{"semver", "bump", "pre", "--label", "beta"}, "1.2.3-beta.1"},
		{"switch from rc1 to beta", "1.2.3-rc1", []string{"semver", "bump", "pre", "--label", "beta"}, "1.2.3-beta.1"},

		// Start new pre-release with --label on final version
		{"add pre-release to final version", "1.2.3", []string{"semver", "bump", "pre", "--label", "rc"}, "1.2.3-rc.1"},
	}
...
func TestCLI_BumpPreCmd_ErrorNoPreRelease(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{"semver", "bump", "pre"})
...
func TestCLI_BumpPreCmd_EarlyFailures(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"semver", "bump", "pre", "--label", "rc"},
			override: func() func() {
				original := clix.FromCommandFn
				clix.FromCommandFn = func(cmd *cli.Command) (bool, error) {
					return false, fmt.Errorf("mock FromCommand error")
...
func TestCLI_BumpPreCmd_PreserveMetadata(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:     []string{"semver", "bump", "pre", "--preserve-meta"},
			expected: "1.2.3-rc.2+build.99",
		},
		{
			name:     "strip metadata by default",
			initial:  "1.2.3-rc.1+build.99",
			args:     []string{"semver", "bump", "pre"},
			expected: "1.2.3-rc.2",
		},
		{
			name:     "add new metadata",
			initial:  "1.2.3-rc.1",
			args:     []string{"semver", "bump", "pre", "--meta", "ci.123"},
