...
/* SUCCESS CASES                                                             */
/* ------------------------------------------------------------------------- */
func TestSetupCLI_BasicStructure(t *testing.T) {
...
func TestSetupCLI_ShowCommand(t *testing.T) {
	tmpDir := t.TempDir()
	versionPath := filepath.Join(tmpDir, ".version")
...
/* ERROR CASES                                                               */
/* ------------------------------------------------------------------------- */
func TestSetupCLI_UsesConfigPath(t *testing.T) {
	tmp := t.TempDir()
	...
	app, err := setupCLI(versionPath) // we still need to pass a dummy path
	if err != nil {
		t.Fatalf("setupCLI failed: %v", err)
	}
...
func TestSetupCLI_InitializeVersionFileError(t *testing.T) {
	tmp := t.TempDir()
