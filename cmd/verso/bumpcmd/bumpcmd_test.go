...
import (
	...
	"github.com/indaco/verso/internal/clix"
	"github.com/indaco/verso/internal/config"
	"github.com/indaco/verso/internal/hooks"
	...
	"github.com/indaco/verso/internal/plugins/commitparser/gitlog"
...
func TestCLI_BumpCommand_Variants(t *testing.T) {
	...
	tests := []struct {
		...
		{"patch bump", "1.2.3", []string{"verso", "bump", "patch"}, "1.2.4"},
		{"minor bump", "1.2.3", []string{"verso", "bump", "minor"}, "1.3.0"},
		{"major bump", "1.2.3", []string{"verso", "bump", "major"}, "2.0.0"},
		{"patch bump after pre-release", "1.2.3-alpha", []string{"verso", "bump", "patch"}, "1.2.4"},
	}
...
func TestCLI_BumpCommand_AutoInitFeedback(t *testing.T) {
	tests := []struct {
		...
		{"patch bump", "1.2.3", []string{"verso", "bump", "patch"}},
		{"minor bump", "1.2.3", []string{"verso", "bump", "minor"}},
		{"major bump", "1.2.3", []string{"verso", "bump", "major"}},
		{"patch bump after pre-release", "1.2.3-alpha", []string{"verso", "bump", "patch"}},
	}
...
func TestCLI_BumpSubcommands_EarlyFailures(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: []string{"verso", "bump", "release"},
			override: func() func() {
				original := clix.FromCommandFn
				clix.FromCommandFn = func(cmd *cli.Command) (bool, error) {
					return false, fmt.Errorf("mock FromCommand error")
...
func TestCLI_BumpReleaseCmd(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:           []string{"verso", "bump", "release", "--preserve-meta"},
...
func TestCLI_BumpAutoCmd(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:     []string{"verso", "bump", "auto", "--preserve-meta"},
...
func TestCLI_BumpAutoCmd_InferredBump(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "auto", "--path", versionPath,
	})
...
func TestCLI_BumpAutoCommand_WithLabelAndMeta(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:    "label=patch",
			initial: "1.2.3",
			args:    []string{"verso", "bump", "auto", "--label", "patch"},
			want:    "1.2.4",
		},
		{
			name:    "label=minor",
			initial: "1.2.3",
			args:    []string{"verso", "bump", "auto", "--label", "minor"},
			want:    "1.3.0",
		},
		{
			name:    "label=major",
			initial: "1.2.3",
			args:    []string{"verso", "bump", "auto", "--label", "major"},
			want:    "2.0.0",
		},
		{
			name:    "label=minor with metadata",
			initial: "1.2.3",
			args:    []string{"verso", "bump", "auto", "--label", "minor", "--meta", "build.42"},
			want:    "1.3.0+build.42",
		},
		{
			name:    "preserve existing metadata",
			initial: "1.2.3+ci.88",
			args:    []string{"verso", "bump", "auto", "--label", "patch", "--preserve-meta"},
			want:    "1.2.4+ci.88",
		},
		{
			name:    "override existing metadata",
			initial: "1.2.3+ci.88",
			args:    []string{"verso", "bump", "auto", "--label", "patch", "--meta", "ci.99"},
			want:    "1.2.4+ci.99",
		},
	}
...
func TestCLI_BumpAutoCmd_PromotePreReleaseWithPreserveMeta(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "auto", "--path", versionPath, "--preserve-meta",
	})
	if err != nil {
		t.Fatalf("expected no error, got: %v", err)
	}
...
func TestBumpReleaseCmd_ErrorOnReadVersion(t *testing.T) {
	...
	appCli := testutils.BuildCLIForTests(cfg.Path, []*cli.Command{Run(cfg)})
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "release", "--path", versionPath,
	})
...
func TestCLI_BumpReleaseCommand_SaveVersionFails(t *testing.T) {
	...
	appCli := testutils.BuildCLIForTests(cfg.Path, []*cli.Command{Run(cfg)})
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "release", "--path", versionPath, "--strict",
	})
...
func TestCLI_BumpAutoCmd_BumpNextFails(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "auto", "--path", versionPath, "--no-infer",
	})
...
func TestCLI_BumpAutoCmd_SaveVersionFails(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "auto", "--path", versionPath, "--strict",
	})
...
func TestCLI_BumpAutoCommand_InvalidLabel(t *testing.T) {
	if os.Getenv("TEST_VERSO_BUMP_AUTO_INVALID_LABEL") == "1" {
		tmp := t.TempDir()
		versionPath := testutils.WriteTempVersionFile(t, tmp, "1.2.3")
		...
		err := appCli.Run(context.Background(), []string{
			"verso", "bump", "auto", "--label", "banana", "--path", versionPath,
		})
		if err != nil {
	...
	cmd.Env = append(os.Environ(), "TEST_VERSO_BUMP_AUTO_INVALID_LABEL=1")
...
func TestCLI_BumpAutoCmd_BumpByLabelFails(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "bump", "auto", "--label", "patch", "--path", versionPath,
	})
...
func TestCLI_BumpPreCmd(t *testing.T) {
	...
	tests := []struct {
		...
		{"increment rc.1 to rc.2", "1.2.3-rc.1", []string{"verso", "bump", "pre"}, "1.2.3-rc.2"},
		{"increment rc.9 to rc.10", "1.2.3-rc.9", []string{"verso", "bump", "pre"}, "1.2.3-rc.10"},
		{"increment alpha.1 to alpha.2", "2.0.0-alpha.1", []string{"verso", "bump", "pre"}, "2.0.0-alpha.2"},

		// Increment existing pre-release (no separator)
		{"increment rc1 to rc2", "1.2.3-rc1", []string{"verso", "bump", "pre"}, "1.2.3-rc2"},
		{"increment beta5 to beta6", "1.2.3-beta5", []string{"verso", "bump", "pre"}, "1.2.3-beta6"},

		// Increment existing pre-release (dash separator)
		{"increment rc-1 to rc-2", "1.2.3-rc-1", []string{"verso", "bump", "pre"}, "1.2.3-rc-2"},

		// Label switch
		{"switch from alpha to beta", "1.2.3-alpha.3", []string{"verso", "bump", "pre", "--label", "beta"}, "1.2.3-beta.1"},
		{"switch from rc1 to beta", "1.2.3-rc1", []string{"verso", "bump", "pre", "--label", "beta"}, "1.2.3-beta.1"},

		// Start new pre-release with --label on final version
		{"add pre-release to final version", "1.2.3", []string{"verso", "bump", "pre", "--label", "rc"}, "1.2.3-rc.1"},
	}
...
func TestCLI_BumpPreCmd_ErrorNoPreRelease(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{"verso", "bump", "pre"})
...
func TestCLI_BumpPreCmd_EarlyFailures(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			args: []string{"verso", "bump", "pre", "--label", "rc"},
			override: func() func() {
				original := clix.FromCommandFn
				clix.FromCommandFn = func(cmd *cli.Command) (bool, error) {
					return false, fmt.Errorf("mock FromCommand error")
...
func TestCLI_BumpPreCmd_PreserveMetadata(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args:     []string{"verso", "bump", "pre", "--preserve-meta"},
			expected: "1.2.3-rc.2+build.99",
		},
		{
			name:     "strip metadata by default",
			initial:  "1.2.3-rc.1+build.99",
			args:     []string{"verso", "bump", "pre"},
			expected: "1.2.3-rc.2",
		},
		{
			name:     "add new metadata",
			initial:  "1.2.3-rc.1",
			args:     []string{"verso", "bump", "pre", "--meta", "ci.123"},
