...
func TestNewCLI_ShowCommand(t *testing.T) {
	...
	err = app.Run(context.Background(), []string{"verso", "show", "--path", versionPath})
...
func TestNewCLI_UsesConfigPath(t *testing.T) {
	...
	yamlPath := filepath.Join(tmp, ".verso.yaml")
	...
	err = app.Run(context.Background(), []string{"verso", "bump", "patch"})
	if err != nil {
...
func TestRunCLI_InitializeVersionFileError(t *testing.T) {
	...
	err = runCLI([]string{"verso", "bump", "patch"})
	if err == nil {
