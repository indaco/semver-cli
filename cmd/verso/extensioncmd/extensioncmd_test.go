...
import (
	...
	"github.com/indaco/verso/api/v0/extensions"
...
func TestExtensionIstallCmd_Success(t *testing.T) {
	...
	// Create .verso.yaml with the required path field
	configContent := `path: .version`
	if err := os.WriteFile(configPath, []byte(configContent), 0644); err != nil {
		t.Fatalf("failed to create .verso.yaml: %v", err)
	}
...
entry: mock-entry`
	...
	output, _ := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{
			"verso", "extension", "install", "--path", extensionDir,
...
func TestExtensionRegisterCmd_MissingPathArgument(t *testing.T) {
	if os.Getenv("TEST_VERSO_EXTENSION_MISSING_PATH") == "1" {
		tmp := t.TempDir()
		...
		err := appCli.Run(context.Background(), []string{"verso", "extension", "install"})
		if err != nil {
	...
	cmd.Env = append(os.Environ(), "TEST_VERSO_EXTENSION_MISSING_PATH=1")
...
func TestExtensionListCmd(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "extension", "list"}, tmpDir)
	})
	if err != nil {
...
`
	...
	if err != nil {
		t.Fatalf("failed to write .verso.yaml: %v", err)
	}
...
func TestExtensionListCmd_LoadConfigError(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		err := appCli.Run(context.Background(), []string{"verso", "extension", "list"})
...
func TestExtensionListCmd_WithRegisteredMetadata(t *testing.T) {
	...
	// Write .verso.yaml with extension that matches registered metadata
...
func TestExtensionRemoveCmd_DeleteFolderVariants(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			extensionsRoot := filepath.Join(tmpDir, ".verso-extensions")
			...
			args := []string{"verso", "extension", "remove", "--name", extensionName}
			if tt.deleteFolder {
...
func TestExtensionRemoveCmd_DeleteFolderFailure(t *testing.T) {
	...
	tmpDir := t.TempDir()
	extensionName := "mock-extension"
	extensionDir := filepath.Join(tmpDir, ".verso-extensions", extensionName)
	extensionConfigPath := filepath.Join(tmpDir, ".verso.yaml")
...
func TestCLI_ExtensionRemove_MissingName(t *testing.T) {
	if os.Getenv("TEST_EXTENSION_REMOVE_MISSING_NAME") == "1" {
		...
		// Write valid .verso.yaml with 1 extension (won't be used, but still required)
  ...
  - name: mock-extension
    ...
    enabled: true`
		...
		err := appCli.Run(context.Background(), []string{
			"verso", "extension", "remove", "--path", configPath,
		})
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
...
func TestExtensionRemoveCmd_LoadConfigError(t *testing.T) {
	...
	// Capture the output
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "extension", "remove", "--name", "mock-plugin"}, tmpDir)
	})
...
func TestExtensionRemoveCmd_PluginNotFound(t *testing.T) {
	...
	// Create a dummy .verso.yaml configuration file with no extension
	content := `extensions: []`
	if err := os.WriteFile(extensionConfigPath, []byte(content), 0644); err != nil {
		t.Fatalf("failed to create .verso.yaml: %v", err)
	}
	...
	// Capture the output
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "extension", "remove", "--name", "mock-extension"}, tmpDir)
	})
...
func TestExtensionRemoveCmd_SaveConfigError(t *testing.T) {
	...
	// Create a dummy .verso.yaml configuration file
	content := `extensions:
  - name: mock-extension
...
func TestExtensionInstallCmd_RegisterLocalExtensionError(t *testing.T) {
	if os.Getenv("TEST_EXTENSION_REGISTER_ERROR") == "1" {
		...
		err := appCli.Run(context.Background(), []string{"verso", "extension", "install", "--path", "/non/existent/path"})
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
