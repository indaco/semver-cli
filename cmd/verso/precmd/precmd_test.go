...
func TestCLI_PreCommand_StaticLabel(t *testing.T) {
	...
	testutils.RunCLITest(t, appCli, []string{"verso", "pre", "--label", "beta.1"}, tmpDir)
...
func TestCLI_PreCommand_Increment(t *testing.T) {
	...
	testutils.RunCLITest(t, appCli, []string{"verso", "pre", "--label", "beta", "--inc"}, tmpDir)
	content := testutils.ReadTempVersionFile(t, tmpDir)
...
func TestCLI_PreCommand_AutoInitFeedback(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "pre", "--label", "alpha"}, tmpDir)
	})
	if err != nil {
...
func TestCLI_PreCommand_InvalidVersion(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "pre", "--label", "alpha", "--path", customPath,
	})
	if err == nil {
...
func TestCLI_PreCommand_SaveVersionFails(t *testing.T) {
	if os.Getenv("TEST_VERSO_PRE_SAVE_FAIL") == "1" {
		tmp := t.TempDir()
		...
		err := appCli.Run(context.Background(), []string{
			"verso", "pre", "--label", "rc", "--path", versionPath,
		})
	...
	cmd.Env = append(os.Environ(), "TEST_VERSO_PRE_SAVE_FAIL=1")
...
func TestCLI_PreCommand_FromCommandFails(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{"verso", "pre", "--label", "beta.1"})
