...
func TestCLI_SetVersionCommandVariants(t *testing.T) {
	...
	tests := []struct {
		...
		{"set version", []string{"verso", "set", "2.5.0"}, "2.5.0"},
		{"set with pre-release", []string{"verso", "set", "3.0.0", "--pre", "beta.2"}, "3.0.0-beta.2"},
		{"set with metadata", []string{"verso", "set", "1.0.0", "--meta", "001"}, "1.0.0+001"},
		{"set with pre and meta", []string{"verso", "set", "1.0.0", "--pre", "alpha.1", "--meta", "exp.sha.5114f85"}, "1.0.0-alpha.1+exp.sha.5114f85"},
	}
...
func TestCLI_SetVersionCommand_InvalidFormat(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{"verso", "set", "invalid.version"})
	if err == nil {
...
func TestCLI_SetVersionCommand_MissingArgument(t *testing.T) {
	if os.Getenv("TEST_VERSO_SET_MISSING_ARG") == "1" {
		tmp := t.TempDir()
		...
		err := appCli.Run(context.Background(), []string{"verso", "set", "--path", versionPath})
		if err != nil {
	...
	cmd.Env = append(os.Environ(), "TEST_VERSO_SET_MISSING_ARG=1")
...
func TestCLI_SetVersionCommand_SaveError(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{
		"verso", "set", "3.0.0", "--path", versionPath,
	})
	if err == nil {
...
func TestCLI_SetVersionCommand_MultiModule_All(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "set", "3.0.0", "--all"}, tmpDir)
	})
	if err != nil {
...
func TestCLI_SetVersionCommand_MultiModule_Specific(t *testing.T) {
	...
	testutils.RunCLITest(t, appCli, []string{"verso", "set", "5.0.0", "--module", "module-a"}, tmpDir)
...
func TestCLI_SetVersionCommand_MultiModule_Quiet(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "set", "6.0.0", "--all", "--quiet"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_SetVersionCommand_MultiModule_JSONFormat(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "set", "7.0.0", "--all", "--format", "json"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_SetVersionCommand_MultiModule_Parallel(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "set", "10.0.0", "--all", "--parallel"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_SetVersionCommand_MultiModule_TextFormat(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "set", "11.0.0", "--all", "--format", "text"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
