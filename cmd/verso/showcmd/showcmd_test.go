...
func TestCLI_ShowCommand(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show"}, tmpDir)
	})
	if err != nil {
...
func TestCLI_ShowCommand_Strict_MissingFile(t *testing.T) {
	if os.Getenv("TEST_VERSO_STRICT") == "1" {
		tmp := t.TempDir()
		...
		err := appCli.Run(context.Background(), []string{"verso", "show", "--strict"})
		if err != nil {
	...
	cmd.Env = append(os.Environ(), "TEST_VERSO_STRICT=1")
...
func TestCLI_ShowCommand_Strict_FileExists(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--strict"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_InvalidVersionContent(t *testing.T) {
	...
	err := appCli.Run(context.Background(), []string{"verso", "show"})
	if err == nil {
...
func TestCLI_ShowCommand_MultiModule_All(t *testing.T) {
	...
	// Test with --all flag
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--all"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_MultiModule_Specific(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--module", "module-a"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_MultiModule_Quiet(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--all", "--quiet"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_MultiModule_JSONFormat(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--all", "--format", "json"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_MultiModule_TableFormat(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--all", "--format", "table"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
...
func TestCLI_ShowCommand_MultiModule_Parallel(t *testing.T) {
	...
	output, err := testutils.CaptureStdout(func() {
		testutils.RunCLITest(t, appCli, []string{"verso", "show", "--all", "--parallel"}, tmpDir)
	})
	if err != nil {
		t.Fatalf("Failed to capture stdout: %v", err)
	}
